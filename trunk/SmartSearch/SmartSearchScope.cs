...
namespace dotnetexplorer.blog.com.WPFIcRtSandFc.SmartSearch
{
    ...
    {
        //Masks used to store string cached object values
        private const string PropertyValueMaskCache = "{0} ";
        private static readonly string EmptyPropertyValueMaskCache = PropertyValueMaskCache.Replace("{0}", "");
        ...
        /// </summary>
        private Dictionary<string, bool> searchTerms = new Dictionary<string, bool>();
        ...
        /// <param name = "filtertypeMode"></param>
        public void ApplySearchCriteria(IEnumerable<string> terms, FilterMode filtertypeMode)
        {
            filterMode = filtertypeMode;
            searchTerms = new Dictionary<string, bool>();
            foreach (string t in terms)
            {
                var exclude = t.Length > 1 ? t.Substring(0, 1) : string.Empty;
                if (exclude.Equals("!"))
                {
                    searchTerms.Add(t.Remove(0, 1), false);
                }
                else
                {
                    searchTerms.Add(t, true);
                }
            }
            ApplySearchCriteria();
        }
        ...
        private void ApplySearchCriteria()
        {
            ...
            Res = 0;
            if (cvs.Filter == null) //if the current filter delegate is empty, set the filter method
                cvs.Filter = FilterSearch;
            else //otherwise
                cvs.Refresh(); //just refresh collection view source
        ...
        private bool FilterSearch(object candidate)
        {
            ...
            else //if OR mode, any one of the search terms can be found in object properties
            {
        ...
        /// </summary>
        /// <param name = "candidate">Candidate for which to retreive values</param>
        /// <returns>Formatted values (serparated given the mask previously declared)</returns>
        private string GetConcatenatedCandidateValue(object candidate)
        {
            sb = new StringBuilder();
            foreach (PropertyFilterValueGetter pfvg in valueGetters)
            {
                sb.AppendFormat(PropertyValueMaskCache, pfvg.GetValue(candidate));
            }
            return sb.ToString().ToLowerInvariant().Replace(EmptyPropertyValueMaskCache, "");
        }
        ...
        /// </summary>
        /// <param name = "o">Removed item</param>
        private void ManageItemRemove(object o)
        {
            substituteSource.Remove(o);
        ...
        /// </summary>
        /// <param name = "o">Added item</param>
        private void ManageItemAdd(object o)
        {
            substituteSource.Add(o);
        ...
        private void ItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (valueGetters.Any(vg => vg.PropertyFilterDescriptor.FieldName == e.PropertyName))
            {
                PropertyFilterValueGetter pf =
                    valueGetters.FirstOrDefault(vg => vg.PropertyFilterDescriptor.FieldName == e.PropertyName);
                if (pf.PropertyFilterDescriptor.MonitorPropertyChanged && substituteSource.Contains(sender))
                {
